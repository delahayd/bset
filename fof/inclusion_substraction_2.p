...
fof(inclusion_substraction_2, conjecture, ![S, T, V, R]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(b), V) = V)
  => ((sort(set(tuple2(a, b)), R) = R) => ((mem(set(tuple2(a, b)), R,
  relation(b, a, S, T)) & subset(b, V, T)) => subset(tuple2(a, b),
  range_substraction(b, a, R, V), R))))))).
