...
fof(equal_image_2, conjecture, ![S, U, T, R]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => ((sort(set(tuple2(a,
  b)), R) = R) => ((mem(set(tuple2(b, a)), inverse(b, a, R), infix_plmngt(a,
  b, T, S)) & subset(a, U, dom(b, a, R))) => infix_eqeq(a, image(a, a,
  semicolon(a, b, a, R, inverse(b, a, R)), U), U))))))).
