...
fof(equal_image_18, conjecture, ![S, U, V, T]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(a), V) = V) => ((sort(set(b),
  T) = T) => ((subset(a, U, S) & (subset(a, V, S) & ~ infix_eqeq(a, inter(a,
  U, V), empty(a)))) => infix_eqeq(b, image(b, a, times(b, a, U, T), V),
  T))))))).
