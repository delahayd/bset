...
tff(monot_image, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), V:set(A), T:set(B), P:set(tuple2(A, B)), R:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T)) & subset(A, V, S))
  => ((subset(tuple2(A, B), R, P) & subset(A, U, V)) => subset(B, image(A,
  B, R, U), image(A, B, P, V))))).
