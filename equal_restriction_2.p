...
tff(equal_restriction_2, conjecture, ![A : $tType, B : $tType]: ![S:set(A),
  U:set(A), T:set(B), R:set(tuple2(A, B))]: ((subset(A, U, S)
  & mem(set(tuple2(B, A)), inverse(A, B, R), infix_plmngt(B, A, T, S)))
  => infix_eqeq(tuple2(A, B), domain_restriction(A, B, U, R),
  range_restriction(A, B, R, image(A, B, R, U))))).
