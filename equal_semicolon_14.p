...
tff(equal_semicolon_14, conjecture, ![A : $tType, B : $tType, C : $tType]:
  ![S:set(A), T:set(B), U:set(C), R:set(tuple2(A, B)), X:B, Y:C]:
  ((mem(set(tuple2(A, B)), R, relation(A, B, S, T)) & (mem(B, X, T)
  & mem(C, Y, U))) => infix_eqeq(tuple2(A, C), semicolon(A, B, C, R,
  singleton(tuple2(B, C), tuple21(C, B, X, Y))), times(A, C, image(B,
  A, inverse(A, B, R), singleton(B, X)), singleton(C, Y))))).
