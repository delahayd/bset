...
fof(inclusion_image_1, conjecture, ![S, U, V, T, R]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(a), V) = V) => ((sort(set(b),
  T) = T) => ((sort(set(tuple2(a, b)), R) = R) => ((mem(set(tuple2(a, b)), R,
  relation(b, a, S, T)) & (subset(a, U, S) & subset(a, V, S))) => subset(b,
  image(b, a, R, inter(a, U, V)), inter(b, image(b, a, R, U), image(b, a, R,
  V)))))))))).
