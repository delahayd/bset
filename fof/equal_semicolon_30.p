...
fof(equal_semicolon_30, conjecture, ![S, V, T, U, R]: ((sort(set(a), S) = S)
  => ((sort(set(a), V) = V) => ((sort(set(b), T) = T) => ((sort(set(c),
  U) = U) => ((sort(set(tuple2(a, b)), R) = R) => ((subset(a, V, S)
  & mem(set(tuple2(a, b)), R, relation(b, a, S, T))) => infix_eqeq(tuple2(c,
  b), semicolon(b, a, c, times(a, c, U, V), R), times(b, c, U, image(b, a, R,
  V)))))))))).
