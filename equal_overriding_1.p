...
tff(equal_overriding_1, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), P:set(tuple2(A, B)), Q:set(tuple2(A, B)), R:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & (mem(set(tuple2(A, B)), Q, relation(A, B, S, T))
  & mem(set(tuple2(A, B)), R, relation(A, B, S, T))))
  => infix_eqeq(tuple2(A, B), infix_lspl(A, B, P, infix_lspl(A, B, Q, R)),
  infix_lspl(A, B, infix_lspl(A, B, P, Q), R)))).
