...
fof(equal_semicolon_9, conjecture, ![S, T, U, W, R, P]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(c), W) = W) => ((sort(set(tuple2(a, b)), R) = R)
  => ((sort(set(tuple2(b, c)), P) = P) => ((mem(set(tuple2(a, b)), R,
  relation(b, a, S, T)) & (mem(set(tuple2(b, c)), P, relation(c, b, T, U))
  & subset(c, W, U))) => infix_eqeq(tuple2(a, c), semicolon(c, b, a, R,
  range_substraction(c, b, P, W)), range_substraction(c, a, semicolon(c, b,
  a, R, P), W)))))))))).
