...
tff(equal_restriction_3, conjecture, ![A : $tType, B : $tType]: ![S:set(A),
  U:set(A), T:set(B), R:set(tuple2(A, B)), P:set(tuple2(B, A))]:
  ((subset(A, U, S) & (mem(set(tuple2(A, B)), R, relation(A, B, S, T))
  & mem(set(tuple2(B, A)), P, relation(B, A, T, U))))
  => infix_eqeq(tuple2(A, A), domain_restriction(A, A, U, semicolon(A, B,
  A, R, P)), semicolon(A, B, A, domain_restriction(A, B, U, R), P)))).
