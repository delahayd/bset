...
fof(equal_image_20, conjecture, ![S, U, V, T, R]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(a), V) = V) => ((sort(set(b),
  T) = T) => ((sort(set(tuple2(a, b)), R) = R) => ((mem(set(tuple2(a, b)), R,
  relation(b, a, S, T)) & (subset(a, U, S) & subset(a, V, S)))
  => infix_eqeq(b, image(b, a, R, union(a, U, V)), union(b, image(b, a, R,
  U), image(b, a, R, V)))))))))).
