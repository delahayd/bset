...
tff(equal_semicolon_31, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), F:set(tuple2(A, B))]: (mem(set(tuple2(A, B)), F, infix_plmngt(A,
  B, S, T)) => infix_eqeq(tuple2(B, B), semicolon(B, A, B, inverse(A, B, F),
  F), id(B, ran(A, B, F))))).
