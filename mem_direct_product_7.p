...
tff(mem_direct_product_7, conjecture, ![A : $tType, B : $tType, C : $tType]:
  ![S:set(A), T:set(B), U:set(C), F:set(tuple2(A, B)), G:set(tuple2(A, C))]:
  ((mem(set(tuple2(A, B)), F, infix_gtmngtgt(A, B, S, T))
  & mem(set(tuple2(A, C)), G, infix_gtmngtgt(A, C, S, U)))
  => mem(set(tuple2(A, tuple2(B, C))), direct_product(A, B, C, F, G),
  infix_gtmngtgt(A, tuple2(B, C), S, times(B, C, T, U))))).
