...
fof(equal_semicolon_22, conjecture, ![S, U, T, P, R]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => ((sort(set(tuple2(a,
  b)), P) = P) => ((sort(set(tuple2(b, a)), R) = R) => ((subset(a, U, S)
  & (mem(set(tuple2(a, b)), P, relation(b, a, S, T)) & mem(set(tuple2(b, a)),
  R, relation(a, b, T, U)))) => infix_eqeq(tuple2(a, a), semicolon(a, b, a,
  domain_substraction(b, a, U, P), R), domain_substraction(a, a, U,
  semicolon(a, b, a, P, R)))))))))).
