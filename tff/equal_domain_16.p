...
tff(equal_domain_16, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), X:A, Y:B]: ((mem(A, X, S) & mem(B, Y, T)) => infix_eqeq(A, dom(A,
  B, singleton(tuple2(A, B), tuple21(B, A, X, Y))), singleton(A, X)))).
