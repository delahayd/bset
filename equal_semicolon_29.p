...
tff(equal_semicolon_29, conjecture, ![A : $tType, B : $tType, C : $tType]:
  ![S:set(A), T:set(B), U:set(C), P:set(tuple2(A, B)), R:set(tuple2(B, C))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & (mem(set(tuple2(B, C)), R, relation(B, C, T, U))
  & infix_eqeq(tuple2(A, B), P, empty(tuple2(A, B)))))
  => infix_eqeq(tuple2(A, C), semicolon(A, B, C, P, R),
  empty(tuple2(A, C))))).
