...
tff(equal_range_3, conjecture, ![A : $tType, B : $tType, C : $tType]: ![S:
  set(A), T:set(B), U:set(C), P:set(tuple2(A, B)), Q:set(tuple2(B, C))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & mem(set(tuple2(B, C)), Q, relation(B, C, T, U))) => infix_eqeq(C, ran(A,
  C, semicolon(A, B, C, P, Q)), image(B, C, Q, ran(A, B, P))))).
