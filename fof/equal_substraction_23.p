...
fof(equal_substraction_23, conjecture, ![S, T, R]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(tuple2(a, b)), R) = R)
  => (mem(set(tuple2(a, b)), R, relation(b, a, S, T)) => infix_eqeq(tuple2(a,
  b), domain_substraction(b, a, dom(b, a, R), R), empty(tuple2(a, b)))))))).
