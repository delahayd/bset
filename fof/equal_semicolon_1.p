...
fof(equal_semicolon_1, conjecture, ![S, T, U, P, Q, R]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(tuple2(b, c)), P) = P) => ((sort(set(tuple2(b, c)), Q) = Q)
  => ((sort(set(tuple2(a, b)), R) = R) => ((mem(set(tuple2(b, c)), P,
  relation(c, b, T, U)) & (mem(set(tuple2(b, c)), Q, relation(c, b, T, U))
  & (mem(set(tuple2(a, b)), R, infix_plmngtgt(b, a, S, T))
  & infix_eqeq(tuple2(a, c), semicolon(c, b, a, R, P), semicolon(c, b, a, R,
  Q))))) => infix_eqeq(tuple2(b, c), P, Q))))))))).
