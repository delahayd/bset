...
fof(equal_semicolon_32, conjecture, ![S, T, R]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(tuple2(a, b)), R) = R)
  => (mem(set(tuple2(b, a)), inverse(b, a, R), infix_plmngt(a, b, T, S))
  => infix_eqeq(tuple2(a, a), semicolon(a, b, a, R, inverse(b, a, R)), id(a,
  dom(b, a, R)))))))).
