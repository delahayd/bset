...
fof(equal_semicolon_29, conjecture, ![S, T, U, P, R]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(c), U) = U) => ((sort(set(tuple2(a,
  b)), P) = P) => ((sort(set(tuple2(b, c)), R) = R) => ((mem(set(tuple2(a,
  b)), P, relation(b, a, S, T)) & (mem(set(tuple2(b, c)), R, relation(c, b,
  T, U)) & infix_eqeq(tuple2(a, b), P, empty(tuple2(a, b)))))
  => infix_eqeq(tuple2(a, c), semicolon(c, b, a, P, R), empty(tuple2(a,
  c)))))))))).
