...
tff(equal_range_10, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), P:set(tuple2(A, B)), Q:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & mem(set(tuple2(A, B)), Q, relation(A, B, S, T))) => infix_eqeq(B, ran(A,
  B, infix_lspl(A, B, P, Q)), union(B, ran(A, B, domain_substraction(A,
  B, dom(A, B, Q), P)), ran(A, B, Q))))).
