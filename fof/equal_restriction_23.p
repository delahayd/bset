...
fof(equal_restriction_23, conjecture, ![S, T, V, F]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(b), V) = V) => ((sort(set(tuple2(a,
  b)), F) = F) => ((mem(set(tuple2(a, b)), F, infix_plmngt(b, a, S, T))
  & subset(b, V, T)) => infix_eqeq(tuple2(a, b), domain_restriction(b, a,
  image(a, b, inverse(b, a, F), V), F), range_restriction(b, a, F, V)))))))).
