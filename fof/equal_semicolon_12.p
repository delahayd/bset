...
fof(equal_semicolon_12, conjecture, ![S, T, U, F, P, Q]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(tuple2(a, b)), F) = F) => ((sort(set(tuple2(b, c)), P) = P)
  => ((sort(set(tuple2(b, c)), Q) = Q) => ((mem(set(tuple2(a, b)), F,
  infix_plmngt(b, a, S, T)) & (mem(set(tuple2(b, c)), P, relation(c, b, T,
  U)) & mem(set(tuple2(b, c)), Q, relation(c, b, T, U))))
  => infix_eqeq(tuple2(a, c), semicolon(c, b, a, F, inter(tuple2(b, c), P,
  Q)), inter(tuple2(a, c), semicolon(c, b, a, F, P), semicolon(c, b, a, F,
  Q))))))))))).
