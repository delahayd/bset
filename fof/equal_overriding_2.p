...
fof(equal_overriding_2, conjecture, ![S, T, P, Q]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(tuple2(a, b)), P) = P)
  => ((sort(set(tuple2(a, b)), Q) = Q) => ((mem(set(tuple2(a, b)), P,
  relation(b, a, S, T)) & (mem(set(tuple2(a, b)), Q, relation(b, a, S, T))
  & infix_eqeq(tuple2(a, b), domain_restriction(b, a, dom(b, a, Q), P),
  domain_restriction(b, a, dom(b, a, P), Q)))) => infix_eqeq(tuple2(a, b),
  infix_lspl(b, a, P, Q), union(tuple2(a, b), P, Q)))))))).
