...
tff(equal_restriction_11, conjecture, ![A : $tType, B : $tType, C : $tType,
  D : $tType]: ![S:set(A), U:set(A), T:set(B), V:set(B), W:set(C), Z:
  set(D), P:set(tuple2(A, C)), Q:set(tuple2(B, D))]: ((subset(A, U, S)
  & (subset(B, V, T) & (mem(set(tuple2(A, C)), P, relation(A, C, S, W))
  & mem(set(tuple2(B, D)), Q, relation(B, D, T, Z)))))
  => infix_eqeq(tuple2(tuple2(A, B), tuple2(C, D)), domain_restriction(
  tuple2(A, B), tuple2(C, D), times(A, B, U, V), parallel_product(A, C, B,
  D, P, Q)), parallel_product(A, C, B, D, domain_restriction(A, C, U, P),
  domain_restriction(B, D, V, Q))))).
