...
fof(equal_semicolon_15, conjecture, ![S, T, U, R, P]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(c), U) = U) => ((sort(set(tuple2(a,
  b)), R) = R) => ((sort(set(tuple2(b, c)), P) = P) => ((mem(set(tuple2(a,
  b)), R, relation(b, a, S, T)) & (mem(set(tuple2(b, c)), P, relation(c, b,
  T, U)) & infix_eqeq(tuple2(b, c), P, empty(tuple2(b, c)))))
  => infix_eqeq(tuple2(a, c), semicolon(c, b, a, R, P), empty(tuple2(a,
  c)))))))))).
