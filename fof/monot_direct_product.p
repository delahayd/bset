...
fof(monot_direct_product, conjecture, ![S, T, U, H, P, K, Q]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(tuple2(a, b)), H) = H) => ((sort(set(tuple2(a, b)), P) = P)
  => ((sort(set(tuple2(a, c)), K) = K) => ((sort(set(tuple2(a, c)), Q) = Q)
  => ((mem(set(tuple2(a, b)), H, relation(b, a, S, T)) & mem(set(tuple2(a,
  c)), K, relation(c, a, S, U))) => ((subset(tuple2(a, b), P, H)
  & subset(tuple2(a, c), Q, K)) => subset(tuple2(a, tuple2(b, c)),
  direct_product(c, b, a, P, Q), direct_product(c, b, a, H, K)))))))))))).
