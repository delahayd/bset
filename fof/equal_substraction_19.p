...
fof(equal_substraction_19, conjecture, ![S, U, V, T]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(a), V) = V) => ((sort(set(b),
  T) = T) => ((subset(a, U, S) & subset(a, V, S)) => infix_eqeq(tuple2(a, b),
  domain_substraction(b, a, U, times(b, a, V, T)), times(b, a, diff(a, V, U),
  T)))))))).
