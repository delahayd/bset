...
fof(equal_restriction_3, conjecture, ![S, U, T, R, P]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => ((sort(set(tuple2(a,
  b)), R) = R) => ((sort(set(tuple2(b, a)), P) = P) => ((subset(a, U, S)
  & (mem(set(tuple2(a, b)), R, relation(b, a, S, T)) & mem(set(tuple2(b, a)),
  P, relation(a, b, T, U)))) => infix_eqeq(tuple2(a, a),
  domain_restriction(a, a, U, semicolon(a, b, a, R, P)), semicolon(a, b, a,
  domain_restriction(b, a, U, R), P))))))))).
