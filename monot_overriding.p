...
tff(monot_overriding, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), H:set(tuple2(A, B)), Q:set(tuple2(A, B)), P:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), H, relation(A, B, S, T))
  & mem(set(tuple2(A, B)), Q, relation(A, B, S, T)))
  => (subset(tuple2(A, B), P, H) => subset(tuple2(A, B), infix_lspl(A, B, P,
  Q), infix_lspl(A, B, H, Q))))).
