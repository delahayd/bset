...
fof(equal_image_6, conjecture, ![S, U, V, T, R]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(a), V) = V) => ((sort(set(b),
  T) = T) => ((sort(set(tuple2(a, b)), R) = R) => ((subset(a, U, S)
  & (mem(set(tuple2(a, b)), R, relation(b, a, S, T)) & subset(a, V, S)))
  => infix_eqeq(b, image(b, a, domain_substraction(b, a, U, R), V), image(b,
  a, R, diff(a, U, V)))))))))).
