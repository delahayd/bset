...
fof(equal_semicolon_14, conjecture, ![S, T, U, R, X, Y]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(tuple2(a, b)), R) = R) => ((sort(b, X) = X) => ((sort(c,
  Y) = Y) => ((mem(set(tuple2(a, b)), R, relation(b, a, S, T)) & (mem(b, X,
  T) & mem(c, Y, U))) => infix_eqeq(tuple2(a, c), semicolon(c, b, a, R,
  singleton(tuple2(b, c), tuple21(b, c, X, Y))), times(c, a, image(a, b,
  inverse(b, a, R), singleton(b, X)), singleton(c, Y))))))))))).
