...
fof(equal_semicolon_34, conjecture, ![S, T, U, V, W, Z, P, Q, H, K]:
  ((sort(set(a), S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(d), V) = V) => ((sort(set(e), W) = W) => ((sort(set(f),
  Z) = Z) => ((sort(set(tuple2(a, b)), P) = P) => ((sort(set(tuple2(c, d)),
  Q) = Q) => ((sort(set(tuple2(b, e)), H) = H) => ((sort(set(tuple2(d, f)),
  K) = K) => ((mem(set(tuple2(a, b)), P, relation(b, a, S, T))
  & (mem(set(tuple2(c, d)), Q, relation(d, c, U, V)) & (mem(set(tuple2(b,
  e)), H, relation(e, b, T, W)) & mem(set(tuple2(d, f)), K, relation(f, d, V,
  Z))))) => infix_eqeq(tuple2(tuple2(a, c), tuple2(e, f)),
  semicolon(tuple2(e, f), tuple2(b, d), tuple2(a, c), parallel_product(d, c,
  b, a, P, Q), parallel_product(f, d, e, b, H, K)), parallel_product(f, c, e,
  a, semicolon(e, b, a, P, H), semicolon(f, d, c, Q, K))))))))))))))).
