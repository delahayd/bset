...
fof(equal_substraction_20, conjecture, ![S, U, V, T, R]: ((sort(set(a),
  S) = S) => ((sort(set(a), U) = U) => ((sort(set(a), V) = V)
  => ((sort(set(b), T) = T) => ((sort(set(tuple2(a, b)), R) = R)
  => ((subset(a, U, S) & (subset(a, V, S) & mem(set(tuple2(a, b)), R,
  relation(b, a, S, T)))) => infix_eqeq(tuple2(a, b), domain_substraction(b,
  a, union(a, U, V), R), inter(tuple2(a, b), domain_substraction(b, a, U, R),
  domain_substraction(b, a, V, R)))))))))).
