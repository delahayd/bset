...
tff(inclusion_dom_1, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), P:set(tuple2(A, B)), Q:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & mem(set(tuple2(A, B)), Q, relation(A, B, S, T))) => subset(A, dom(A,
  B, inter(tuple2(A, B), P, Q)), inter(A, dom(A, B, P), dom(A, B, Q))))).
