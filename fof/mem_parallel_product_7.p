...
fof(mem_parallel_product_7, conjecture, ![S, T, U, V, F, G]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(d), V) = V) => ((sort(set(tuple2(a, b)), F) = F)
  => ((sort(set(tuple2(c, d)), G) = G) => ((mem(set(tuple2(a, b)), F,
  infix_mnmngtgt(b, a, S, T)) & mem(set(tuple2(c, d)), G, infix_mnmngtgt(d,
  c, U, V))) => mem(set(tuple2(tuple2(a, c), tuple2(b, d))),
  parallel_product(d, c, b, a, F, G), infix_mnmngtgt(tuple2(b, d), tuple2(a,
  c), times(c, a, S, U), times(d, b, T, V))))))))))).
