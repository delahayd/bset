...
tff(equal_image_21, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), V:set(A), T:set(B), R:set(tuple2(A, B))]:
  ((mem(set(tuple2(B, A)), inverse(A, B, R), infix_plmngt(B, A, T, S))
  & (subset(A, U, S) & subset(A, V, S))) => infix_eqeq(B, image(A, B, R,
  inter(A, U, V)), inter(B, image(A, B, R, U), image(A, B, R, V))))).
