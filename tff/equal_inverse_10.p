...
tff(equal_inverse_10, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), P:set(tuple2(A, B)), Q:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & mem(set(tuple2(A, B)), Q, relation(A, B, S, T)))
  => infix_eqeq(tuple2(B, A), inverse(A, B, union(tuple2(A, B), P, Q)),
  union(tuple2(B, A), inverse(A, B, P), inverse(A, B, Q))))).
