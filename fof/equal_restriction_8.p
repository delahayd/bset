...
fof(equal_restriction_8, conjecture, ![S, U, T, W, R]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => ((sort(set(b),
  W) = W) => ((sort(set(tuple2(a, b)), R) = R) => ((subset(a, U, S)
  & (mem(set(tuple2(a, b)), R, relation(b, a, S, T)) & subset(b, W, T)))
  => infix_eqeq(tuple2(a, b), domain_restriction(b, a, U,
  range_substraction(b, a, R, W)), range_substraction(b, a,
  domain_restriction(b, a, U, R), W))))))))).
