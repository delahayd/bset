...
tff(equal_image_7, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), T:set(B), V:set(B), R:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), R, relation(A, B, S, T)) & (subset(B, V, T)
  & subset(A, U, S))) => infix_eqeq(B, image(A, B, range_substraction(A,
  B, R, V), U), diff(B, image(A, B, R, U), V)))).
