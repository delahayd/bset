...
fof(equal_semicolon_28, conjecture, ![S, T, U, X, Y, R]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U) => ((sort(a,
  X) = X) => ((sort(b, Y) = Y) => ((sort(set(tuple2(b, c)), R) = R)
  => ((mem(a, X, S) & (mem(b, Y, T) & mem(set(tuple2(b, c)), R, relation(c,
  b, T, U)))) => infix_eqeq(tuple2(a, c), semicolon(c, b, a,
  singleton(tuple2(a, b), tuple21(a, b, X, Y)), R), times(c, a, singleton(a,
  X), image(c, b, R, singleton(b, Y)))))))))))).
