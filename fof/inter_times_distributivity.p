...
fof(inter_times_distributivity, conjecture, ![A, B, T]: ((sort(set(a),
  A) = A) => ((sort(set(a), B) = B) => ((sort(set(a), T) = T)
  => infix_eqeq(tuple2(a, a), times(a, a, inter(a, A, B), T), inter(tuple2(a,
  a), times(a, a, A, T), times(a, a, B, T))))))).
