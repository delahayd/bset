...
tff(equal_substraction_17, conjecture, ![A : $tType, B : $tType]: ![S:
  set(A), U:set(A), T:set(B), X:A, Y:B]: ((subset(A, U, S) & (mem(A, X, S)
  & (mem(B, Y, T) & mem(A, X, U))))
  => infix_eqeq(tuple2(A, B), domain_substraction(A, B, U,
  singleton(tuple2(A, B), tuple21(B, A, X, Y))), empty(tuple2(A, B))))).
