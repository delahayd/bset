...
tff(equal_substraction_19, conjecture, ![A : $tType, B : $tType]: ![S:
  set(A), U:set(A), V:set(A), T:set(B)]: ((subset(A, U, S) & subset(A, V, S))
  => infix_eqeq(tuple2(A, B), domain_substraction(A, B, U, times(A, B, V,
  T)), times(A, B, diff(A, V, U), T)))).
