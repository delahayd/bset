...
tff(monot_restriction_1, conjecture, ![A : $tType, B : $tType]: ![S:set(A),
  U:set(A), V:set(A), T:set(B), P:set(tuple2(A, B)), R:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T)) & subset(A, V, S))
  => ((subset(A, U, V) & subset(tuple2(A, B), R, P))
  => subset(tuple2(A, B), domain_restriction(A, B, U, R),
  domain_restriction(A, B, V, P))))).
