...
tff(equal_image_17, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), T:set(B), R:set(tuple2(A, B))]: ((mem(set(tuple2(A, B)), R,
  relation(A, B, S, T)) & infix_eqeq(A, inter(A, dom(A, B, R), U), empty(A)))
  => infix_eqeq(B, image(A, B, R, U), empty(B)))).
