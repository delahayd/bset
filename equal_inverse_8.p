...
tff(equal_inverse_8, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), P:set(tuple2(A, B)), Q:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & mem(set(tuple2(A, B)), Q, relation(A, B, S, T)))
  => infix_eqeq(tuple2(B, A), inverse(A, B, infix_lspl(A, B, P, Q)),
  union(tuple2(B, A), range_substraction(B, A, inverse(A, B, P), dom(A,
  B, Q)), inverse(A, B, Q))))).
