...
tff(equal_semicolon_23, conjecture, ![A : $tType, B : $tType, C : $tType]:
  ![S:set(A), T:set(B), V:set(B), U:set(C), P:set(tuple2(A, B)), R:
  set(tuple2(B, C))]: ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & (subset(B, V, T) & mem(set(tuple2(B, C)), R, relation(B, C, T, U))))
  => infix_eqeq(tuple2(A, C), semicolon(A, B, C, range_substraction(A, B, P,
  V), R), semicolon(A, B, C, P, domain_substraction(B, C, V, R))))).
