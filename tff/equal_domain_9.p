...
tff(equal_domain_9, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), V:set(B), F:set(tuple2(A, B))]: ((mem(set(tuple2(A, B)), F,
  infix_plmngt(A, B, S, T)) & subset(B, V, T)) => infix_eqeq(A, dom(A,
  B, range_substraction(A, B, F, V)), diff(A, dom(A, B, F), image(B,
  A, inverse(A, B, F), V))))).
