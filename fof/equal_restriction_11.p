...
fof(equal_restriction_11, conjecture, ![S, U, T, V, W, Z, P, Q]:
  ((sort(set(a), S) = S) => ((sort(set(a), U) = U) => ((sort(set(b), T) = T)
  => ((sort(set(b), V) = V) => ((sort(set(c), W) = W) => ((sort(set(d),
  Z) = Z) => ((sort(set(tuple2(a, c)), P) = P) => ((sort(set(tuple2(b, d)),
  Q) = Q) => ((subset(a, U, S) & (subset(b, V, T) & (mem(set(tuple2(a, c)),
  P, relation(c, a, S, W)) & mem(set(tuple2(b, d)), Q, relation(d, b, T,
  Z))))) => infix_eqeq(tuple2(tuple2(a, b), tuple2(c, d)),
  domain_restriction(tuple2(c, d), tuple2(a, b), times(b, a, U, V),
  parallel_product(d, b, c, a, P, Q)), parallel_product(d, b, c, a,
  domain_restriction(c, a, U, P), domain_restriction(d, b, V, Q))))))))))))).
