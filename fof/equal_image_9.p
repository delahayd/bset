...
fof(equal_image_9, conjecture, ![S, U, T, V, P, Q]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => ((sort(set(c),
  V) = V) => ((sort(set(tuple2(a, b)), P) = P) => ((sort(set(tuple2(a, c)),
  Q) = Q) => ((mem(set(tuple2(a, b)), P, relation(b, a, S, T))
  & (mem(set(tuple2(a, c)), Q, relation(c, a, S, V)) & subset(a, U, S)))
  => infix_eqeq(tuple2(b, c), image(tuple2(b, c), a, direct_product(c, b, a,
  P, Q), U), semicolon(c, a, b, inverse(b, a, P), domain_restriction(c, a, U,
  Q))))))))))).
