...
tff(equal_inverse_14, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), R:set(tuple2(A, B))]: ((mem(set(tuple2(A, B)), R, relation(A, B, S,
  T)) & infix_eqeq(tuple2(A, B), R, empty(tuple2(A, B))))
  => infix_eqeq(tuple2(B, A), inverse(A, B, R), empty(tuple2(B, A))))).
