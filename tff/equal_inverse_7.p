...
tff(equal_inverse_7, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), V:set(B), R:set(tuple2(A, B))]: ((mem(set(tuple2(A, B)), R,
  relation(A, B, S, T)) & subset(B, V, T))
  => infix_eqeq(tuple2(B, A), inverse(A, B, range_substraction(A, B, R, V)),
  domain_substraction(B, A, V, inverse(A, B, R))))).
