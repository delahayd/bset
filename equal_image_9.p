...
tff(equal_image_9, conjecture, ![A : $tType, B : $tType, C : $tType]: ![S:
  set(A), U:set(A), T:set(B), V:set(C), P:set(tuple2(A, B)), Q:
  set(tuple2(A, C))]: ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & (mem(set(tuple2(A, C)), Q, relation(A, C, S, V)) & subset(A, U, S)))
  => infix_eqeq(tuple2(B, C), image(A, tuple2(B, C), direct_product(A, B,
  C, P, Q), U), semicolon(B, A, C, inverse(A, B, P), domain_restriction(A,
  C, U, Q))))).
