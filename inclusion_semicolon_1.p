...
tff(inclusion_semicolon_1, conjecture, ![A : $tType, B : $tType, C : $tType]:
  ![S:set(A), T:set(B), U:set(C), R:set(tuple2(A, B)), P:set(tuple2(B, C)),
  Q:set(tuple2(B, C))]: ((mem(set(tuple2(A, B)), R, relation(A, B, S, T))
  & (mem(set(tuple2(B, C)), P, relation(B, C, T, U))
  & mem(set(tuple2(B, C)), Q, relation(B, C, T, U))))
  => subset(tuple2(A, C), semicolon(A, B, C, R, inter(tuple2(B, C), P, Q)),
  inter(tuple2(A, C), semicolon(A, B, C, R, P), semicolon(A, B, C, R, Q))))).
