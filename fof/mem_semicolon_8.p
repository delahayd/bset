...
fof(mem_semicolon_8, conjecture, ![S, T, U, F, G]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(c), U) = U) => ((sort(set(tuple2(a,
  b)), F) = F) => ((sort(set(tuple2(b, c)), G) = G) => ((mem(set(tuple2(a,
  b)), F, infix_gtmngtgt(b, a, S, T)) & mem(set(tuple2(b, c)), G,
  infix_gtmngtgt(c, b, T, U))) => mem(set(tuple2(a, c)), semicolon(c, b, a,
  F, G), infix_gtmngtgt(c, a, S, U))))))))).
