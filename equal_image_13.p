...
tff(equal_image_13, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), T:set(B), P:set(tuple2(A, B)), Q:set(tuple2(A, B))]:
  ((mem(set(tuple2(B, A)), inverse(A, B, P), infix_plmngt(B, A, T, S))
  & (mem(set(tuple2(B, A)), inverse(A, B, Q), infix_plmngt(B, A, T, S))
  & (subset(A, U, S) & infix_eqeq(tuple2(A, B), range_restriction(A, B, P,
  ran(A, B, Q)), range_restriction(A, B, Q, ran(A, B, P))))))
  => infix_eqeq(B, image(A, B, inter(tuple2(A, B), P, Q), U),
  inter(B, image(A, B, P, U), image(A, B, Q, U))))).
