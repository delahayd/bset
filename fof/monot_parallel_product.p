...
fof(monot_parallel_product, conjecture, ![S, T, U, V, H, P, K, Q]:
  ((sort(set(a), S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(d), V) = V) => ((sort(set(tuple2(a, b)), H) = H)
  => ((sort(set(tuple2(a, b)), P) = P) => ((sort(set(tuple2(c, d)), K) = K)
  => ((sort(set(tuple2(c, d)), Q) = Q) => ((mem(set(tuple2(a, b)), H,
  relation(b, a, S, T)) & mem(set(tuple2(c, d)), K, relation(d, c, U, V)))
  => ((subset(tuple2(a, b), P, H) & subset(tuple2(c, d), Q, K))
  => subset(tuple2(tuple2(a, c), tuple2(b, d)), parallel_product(d, c, b, a,
  P, Q), parallel_product(d, c, b, a, H, K))))))))))))).
