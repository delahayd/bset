...
fof(inclusion_id_2, conjecture, ![S, T, R]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(tuple2(a, b)), R) = R)
  => (mem(set(tuple2(a, b)), R, relation(b, a, S, T)) => subset(tuple2(b, b),
  id(b, ran(b, a, R)), semicolon(b, a, b, inverse(b, a, R), R))))))).
