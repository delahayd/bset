...
fof(equal_restriction_5, conjecture, ![S, U, V, T, R]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(a), V) = V) => ((sort(set(b),
  T) = T) => ((sort(set(tuple2(a, b)), R) = R) => ((subset(a, U, S)
  & (subset(a, V, S) & mem(set(tuple2(a, b)), R, relation(b, a, S, T))))
  => infix_eqeq(tuple2(a, b), domain_restriction(b, a, U,
  domain_restriction(b, a, V, R)), domain_restriction(b, a, inter(a, U, V),
  R))))))))).
