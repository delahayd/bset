...
fof(equal_domain_9, conjecture, ![S, T, V, F]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(b), V) = V) => ((sort(set(tuple2(a,
  b)), F) = F) => ((mem(set(tuple2(a, b)), F, infix_plmngt(b, a, S, T))
  & subset(b, V, T)) => infix_eqeq(a, dom(b, a, range_substraction(b, a, F,
  V)), diff(a, dom(b, a, F), image(a, b, inverse(b, a, F), V))))))))).
