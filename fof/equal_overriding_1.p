...
fof(equal_overriding_1, conjecture, ![S, T, P, Q, R]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(tuple2(a, b)), P) = P)
  => ((sort(set(tuple2(a, b)), Q) = Q) => ((sort(set(tuple2(a, b)), R) = R)
  => ((mem(set(tuple2(a, b)), P, relation(b, a, S, T)) & (mem(set(tuple2(a,
  b)), Q, relation(b, a, S, T)) & mem(set(tuple2(a, b)), R, relation(b, a, S,
  T)))) => infix_eqeq(tuple2(a, b), infix_lspl(b, a, P, infix_lspl(b, a, Q,
  R)), infix_lspl(b, a, infix_lspl(b, a, P, Q), R))))))))).
