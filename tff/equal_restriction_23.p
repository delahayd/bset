...
tff(equal_restriction_23, conjecture, ![A : $tType, B : $tType]: ![S:
  set(A), T:set(B), V:set(B), F:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), F, infix_plmngt(A, B, S, T)) & subset(B, V, T))
  => infix_eqeq(tuple2(A, B), domain_restriction(A, B, image(B, A, inverse(A,
  B, F), V), F), range_restriction(A, B, F, V)))).
