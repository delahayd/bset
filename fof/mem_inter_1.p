...
fof(mem_inter_1, conjecture, ![S, T, F, G]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(tuple2(a, b)), F) = F)
  => ((sort(set(tuple2(a, b)), G) = G) => ((mem(set(tuple2(a, b)), F,
  infix_plmngt(b, a, S, T)) & mem(set(tuple2(a, b)), G, relation(b, a, S,
  T))) => mem(set(tuple2(a, b)), inter(tuple2(a, b), F, G), infix_plmngt(b,
  a, S, T)))))))).
