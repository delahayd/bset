...
fof(monot_substraction_1, conjecture, ![S, U, V, T, P, R]: ((sort(set(a),
  S) = S) => ((sort(set(a), U) = U) => ((sort(set(a), V) = V)
  => ((sort(set(b), T) = T) => ((sort(set(tuple2(a, b)), P) = P)
  => ((sort(set(tuple2(a, b)), R) = R) => ((mem(set(tuple2(a, b)), P,
  relation(b, a, S, T)) & subset(a, U, S)) => ((subset(a, V, U)
  & subset(tuple2(a, b), R, P)) => subset(tuple2(a, b),
  domain_substraction(b, a, U, R), domain_substraction(b, a, V, P))))))))))).
