...
tff(equal_substraction_8, conjecture, ![A : $tType, B : $tType]: ![S:
  set(A), U:set(A), T:set(B), W:set(B), R:set(tuple2(A, B))]: ((subset(A, U,
  S) & (mem(set(tuple2(A, B)), R, relation(A, B, S, T)) & subset(B, W, T)))
  => infix_eqeq(tuple2(A, B), domain_substraction(A, B, U,
  range_substraction(A, B, R, W)), range_substraction(A,
  B, domain_substraction(A, B, U, R), W)))).
