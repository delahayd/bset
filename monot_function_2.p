...
tff(monot_function_2, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), U:set(A), V:set(B)]: ((subset(A, U, S) & subset(B, V, T))
  => subset(set(tuple2(A, B)), infix_gtplgt(A, B, U, V), infix_gtplgt(A,
  B, S, T)))).
