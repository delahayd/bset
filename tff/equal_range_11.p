...
tff(equal_range_11, conjecture, ![A : $tType, B : $tType, C : $tType]: ![S:
  set(A), T:set(B), U:set(C), P:set(tuple2(A, B)), Q:set(tuple2(A, C))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & mem(set(tuple2(A, C)), Q, relation(A, C, S, U)))
  => infix_eqeq(tuple2(B, C), ran(A, tuple2(B, C), direct_product(A, B, C, P,
  Q)), semicolon(B, A, C, inverse(A, B, P), Q)))).
