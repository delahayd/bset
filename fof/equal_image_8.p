...
fof(equal_image_8, conjecture, ![S, U, T, P, Q]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => ((sort(set(tuple2(a,
  b)), P) = P) => ((sort(set(tuple2(a, b)), Q) = Q) => ((mem(set(tuple2(a,
  b)), P, relation(b, a, S, T)) & (mem(set(tuple2(a, b)), Q, relation(b, a,
  S, T)) & subset(a, U, S))) => infix_eqeq(b, image(b, a, infix_lspl(b, a, P,
  Q), U), union(b, image(b, a, domain_substraction(b, a, dom(b, a, Q), P),
  U), image(b, a, Q, U)))))))))).
