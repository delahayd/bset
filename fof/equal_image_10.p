...
fof(equal_image_10, conjecture, ![S, U, T, W, V, Z, P, Q]: ((sort(set(a),
  S) = S) => ((sort(set(a), U) = U) => ((sort(set(b), T) = T)
  => ((sort(set(c), W) = W) => ((sort(set(c), V) = V) => ((sort(set(d),
  Z) = Z) => ((sort(set(tuple2(a, b)), P) = P) => ((sort(set(tuple2(c, d)),
  Q) = Q) => ((mem(set(tuple2(a, b)), P, relation(b, a, S, T))
  & (mem(set(tuple2(c, d)), Q, relation(d, c, W, Z)) & (subset(a, U, S)
  & subset(c, V, W)))) => infix_eqeq(tuple2(b, d), image(tuple2(b, d),
  tuple2(a, c), parallel_product(d, c, b, a, P, Q), times(c, a, U, V)),
  times(d, b, image(b, a, P, U), image(d, c, Q, V))))))))))))).
