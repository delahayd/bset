...
tff(equal_restriction_24, conjecture, ![A : $tType, B : $tType]: ![S:
  set(A), T:set(B), X:A, R:set(tuple2(A, B))]: ((mem(A, X, S)
  & mem(set(tuple2(A, B)), R, relation(A, B, S, T)))
  => infix_eqeq(tuple2(A, B), domain_restriction(A, B, singleton(A, X), R),
  times(A, B, singleton(A, X), image(A, B, R, singleton(A, X)))))).
