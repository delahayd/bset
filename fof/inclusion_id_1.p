...
fof(inclusion_id_1, conjecture, ![S, T, R]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(tuple2(a, b)), R) = R)
  => (mem(set(tuple2(a, b)), R, relation(b, a, S, T)) => subset(tuple2(a, a),
  id(a, dom(b, a, R)), semicolon(a, b, a, R, inverse(b, a, R)))))))).
