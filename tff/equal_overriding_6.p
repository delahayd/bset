...
tff(equal_overriding_6, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), T:set(B), X:A, Y:B]: ((subset(A, U, S) & (mem(B, Y, T) & mem(A, X,
  S))) => infix_eqeq(tuple2(A, B), infix_lspl(A, B, times(A, B, U,
  singleton(B, Y)), singleton(tuple2(A, B), tuple21(B, A, X, Y))), times(A,
  B, union(A, U, singleton(A, X)), singleton(B, Y))))).
