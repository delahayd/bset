...
tff(equal_semicolon_3, conjecture, ![A : $tType, B : $tType, C : $tType,
  D : $tType]: ![S:set(A), T:set(B), U:set(C), V:set(D), P:set(tuple2(B, C)),
  Q:set(tuple2(C, D)), R:set(tuple2(A, B))]: ((mem(set(tuple2(A, B)), R,
  relation(A, B, S, T)) & (mem(set(tuple2(B, C)), P, relation(B, C, T, U))
  & mem(set(tuple2(C, D)), Q, relation(C, D, U, V))))
  => infix_eqeq(tuple2(A, D), semicolon(A, B, D, R, semicolon(B, C, D, P,
  Q)), semicolon(A, C, D, semicolon(A, B, C, R, P), Q)))).
