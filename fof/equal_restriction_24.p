...
fof(equal_restriction_24, conjecture, ![S, T, X, R]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(a, X) = X) => ((sort(set(tuple2(a, b)),
  R) = R) => ((mem(a, X, S) & mem(set(tuple2(a, b)), R, relation(b, a, S,
  T))) => infix_eqeq(tuple2(a, b), domain_restriction(b, a, singleton(a, X),
  R), times(b, a, singleton(a, X), image(b, a, R, singleton(a, X)))))))))).
