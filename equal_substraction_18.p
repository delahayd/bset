...
tff(equal_substraction_18, conjecture, ![A : $tType, B : $tType]: ![S:
  set(A), U:set(A), T:set(B), R:set(tuple2(A, B))]: ((subset(A, U, S)
  & (mem(set(tuple2(A, B)), R, relation(A, B, S, T)) & subset(A, dom(A,
  B, R), U))) => infix_eqeq(tuple2(A, B), domain_substraction(A, B, U, R),
  empty(tuple2(A, B))))).
