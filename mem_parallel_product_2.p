...
tff(mem_parallel_product_2, conjecture, ![A : $tType, B : $tType, C : $tType,
  D : $tType]: ![S:set(A), T:set(B), U:set(C), V:set(D), F:set(tuple2(A, B)),
  G:set(tuple2(C, D))]: ((mem(set(tuple2(A, B)), F, infix_plmngt(A, B, S, T))
  & mem(set(tuple2(C, D)), G, infix_plmngt(C, D, U, V)))
  => mem(set(tuple2(tuple2(A, C), tuple2(B, D))), parallel_product(A, B, C,
  D, F, G), infix_plmngt(tuple2(A, C), tuple2(B, D), times(A, C, S, U),
  times(B, D, T, V))))).
