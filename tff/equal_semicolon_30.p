...
tff(equal_semicolon_30, conjecture, ![A : $tType, B : $tType, C : $tType]:
  ![S:set(A), V:set(A), T:set(B), U:set(C), R:set(tuple2(A, B))]:
  ((subset(A, V, S) & mem(set(tuple2(A, B)), R, relation(A, B, S, T)))
  => infix_eqeq(tuple2(C, B), semicolon(C, A, B, times(C, A, U, V), R),
  times(C, B, U, image(A, B, R, V))))).
