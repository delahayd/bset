...
tff(equal_image_19, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), V:set(A), T:set(B)]: ((subset(A, U, S) & (subset(A, V, S)
  & infix_eqeq(A, inter(A, U, V), empty(A)))) => infix_eqeq(B, image(A,
  B, times(A, B, U, T), V), empty(B)))).
