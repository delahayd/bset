...
tff(equal_semicolon_8, conjecture, ![A : $tType, B : $tType, C : $tType]:
  ![S:set(A), T:set(B), V:set(B), U:set(C), R:set(tuple2(A, B)), P:
  set(tuple2(B, C))]: ((mem(set(tuple2(A, B)), R, relation(A, B, S, T))
  & (subset(B, V, T) & mem(set(tuple2(B, C)), P, relation(B, C, T, U))))
  => infix_eqeq(tuple2(A, C), semicolon(A, B, C, R, domain_substraction(B,
  C, V, P)), semicolon(A, B, C, range_substraction(A, B, R, V), P)))).
