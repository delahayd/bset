...
tff(equal_semicolon_34, conjecture, ![A : $tType, B : $tType, C : $tType,
  D : $tType, E : $tType, F : $tType]: ![S:set(A), T:set(B), U:set(C), V:
  set(D), W:set(E), Z:set(F), P:set(tuple2(A, B)), Q:set(tuple2(C, D)), H:
  set(tuple2(B, E)), K:set(tuple2(D, F))]: ((mem(set(tuple2(A, B)), P,
  relation(A, B, S, T)) & (mem(set(tuple2(C, D)), Q, relation(C, D, U, V))
  & (mem(set(tuple2(B, E)), H, relation(B, E, T, W))
  & mem(set(tuple2(D, F)), K, relation(D, F, V, Z)))))
  => infix_eqeq(tuple2(tuple2(A, C), tuple2(E, F)), semicolon(tuple2(A, C),
  tuple2(B, D), tuple2(E, F), parallel_product(A, B, C, D, P, Q),
  parallel_product(B, E, D, F, H, K)), parallel_product(A, E, C,
  F, semicolon(A, B, E, P, H), semicolon(C, D, F, Q, K))))).
