...
tff(equal_overriding_2, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), P:set(tuple2(A, B)), Q:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & (mem(set(tuple2(A, B)), Q, relation(A, B, S, T))
  & infix_eqeq(tuple2(A, B), domain_restriction(A, B, dom(A, B, Q), P),
  domain_restriction(A, B, dom(A, B, P), Q))))
  => infix_eqeq(tuple2(A, B), infix_lspl(A, B, P, Q), union(tuple2(A, B), P,
  Q)))).
