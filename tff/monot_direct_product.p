...
tff(monot_direct_product, conjecture, ![A : $tType, B : $tType, C : $tType]:
  ![S:set(A), T:set(B), U:set(C), H:set(tuple2(A, B)), P:set(tuple2(A, B)),
  K:set(tuple2(A, C)), Q:set(tuple2(A, C))]: ((mem(set(tuple2(A, B)), H,
  relation(A, B, S, T)) & mem(set(tuple2(A, C)), K, relation(A, C, S, U)))
  => ((subset(tuple2(A, B), P, H) & subset(tuple2(A, C), Q, K))
  => subset(tuple2(A, tuple2(B, C)), direct_product(A, B, C, P, Q),
  direct_product(A, B, C, H, K))))).
