...
tff(equal_semicolon_16, conjecture, ![A : $tType, B : $tType, C : $tType]:
  ![S:set(A), T:set(B), U:set(B), V:set(C), R:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), R, relation(A, B, S, T)) & subset(B, U, T))
  => infix_eqeq(tuple2(A, C), semicolon(A, B, C, R, times(B, C, U, V)),
  times(A, C, image(B, A, inverse(A, B, R), U), V)))).
