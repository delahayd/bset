...
tff(equal_domain_14, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), F:set(tuple2(A, B)), G:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), F, infix_plmngt(A, B, S, T))
  & (mem(set(tuple2(A, B)), G, infix_plmngt(A, B, S, T))
  & infix_eqeq(tuple2(A, B), domain_restriction(A, B, dom(A, B, F), G),
  domain_restriction(A, B, dom(A, B, G), F)))) => infix_eqeq(A, dom(A,
  B, inter(tuple2(A, B), F, G)), inter(A, dom(A, B, F), dom(A, B, G))))).
