...
fof(equal_image_11, conjecture, ![S, T, U, P, Q]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(c), U) = U) => ((sort(set(tuple2(a,
  b)), P) = P) => ((sort(set(tuple2(b, c)), Q) = Q) => ((mem(set(tuple2(a,
  b)), P, relation(b, a, S, T)) & mem(set(tuple2(b, c)), Q, relation(c, b, T,
  U))) => infix_eqeq(tuple2(a, c), image(tuple2(a, c), tuple2(a, b),
  parallel_product(c, b, a, a, id(a, S), Q), P), semicolon(c, b, a, P,
  Q))))))))).
