...
tff(equal_overriding_5, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), V:set(B), X:A, Y:B]: ((mem(A, X, S) & (subset(B, V, T) & mem(B, Y,
  T))) => infix_eqeq(tuple2(A, B), infix_lspl(A, B, times(A,
  B, singleton(A, X), V), singleton(tuple2(A, B), tuple21(B, A, X, Y))),
  singleton(tuple2(A, B), tuple21(B, A, X, Y))))).
