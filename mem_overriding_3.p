...
tff(mem_overriding_3, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  ...
  & infix_eqeq(B, inter(B, ran(A, B, G), ran(A, B, domain_substraction(A,
  B, dom(A, B, G), F))), empty(B)))) => mem(set(tuple2(A, B)), infix_lspl(A,
  B, F, G), infix_gtplgt(A, B, S, T)))).
