...
tff(equal_inverse_4, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), T:set(B), R:set(tuple2(A, B))]: ((subset(A, U, S)
  & mem(set(tuple2(A, B)), R, relation(A, B, S, T)))
  => infix_eqeq(tuple2(B, A), inverse(A, B, domain_restriction(A, B, U, R)),
  range_restriction(B, A, inverse(A, B, R), U)))).
