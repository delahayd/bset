...
fof(mem_substraction_1, conjecture, ![S, T, U, R]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(a), U) = U) => ((sort(set(tuple2(a,
  b)), R) = R) => ((subset(a, U, S) & mem(set(tuple2(a, b)), R, relation(b,
  a, S, T))) => mem(set(tuple2(a, b)), domain_substraction(b, a, U, R),
  relation(b, a, diff(a, S, U), T)))))))).
