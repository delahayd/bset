...
tff(equal_semicolon_32, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), R:set(tuple2(A, B))]: (mem(set(tuple2(B, A)), inverse(A, B, R),
  infix_plmngt(B, A, T, S)) => infix_eqeq(tuple2(A, A), semicolon(A, B, A, R,
  inverse(A, B, R)), id(A, dom(A, B, R))))).
