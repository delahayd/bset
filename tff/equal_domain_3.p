...
tff(equal_domain_3, conjecture, ![A : $tType, B : $tType, C : $tType]: ![S:
  set(A), T:set(B), U:set(C), P:set(tuple2(A, B)), Q:set(tuple2(B, C))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & mem(set(tuple2(B, C)), Q, relation(B, C, T, U))) => infix_eqeq(A, dom(A,
  C, semicolon(A, B, C, P, Q)), image(B, A, inverse(A, B, P), dom(B,
  C, Q))))).
