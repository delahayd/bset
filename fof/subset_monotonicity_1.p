...
fof(subset_monotonicity_1, conjecture, ![S, T, U, V]: ((sort(set(a), S) = S)
  => ((sort(set(a), T) = T) => ((sort(set(a), U) = U) => ((sort(set(a),
  V) = V) => ((subset(a, U, S) & subset(a, V, T)) => subset(tuple2(a, a),
  times(a, a, U, V), times(a, a, S, T)))))))).
