...
fof(inter_de_morgan, conjecture, ![S, A, B]: ((sort(set(a), S) = S)
  => ((sort(set(a), A) = A) => ((sort(set(a), B) = B) => ((subset(a, A, S)
  & subset(a, B, S)) => infix_eqeq(a, diff(a, S, inter(a, A, B)), union(a,
  diff(a, S, A), diff(a, S, B)))))))).
