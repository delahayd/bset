...
tff(monot_substraction_2, conjecture, ![A : $tType, B : $tType]: ![S:
  set(A), T:set(B), U:set(B), V:set(B), P:set(tuple2(A, B)), R:
  set(tuple2(A, B))]: ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & subset(B, U, T)) => ((subset(tuple2(A, B), R, P) & subset(B, V, U))
  => subset(tuple2(A, B), range_substraction(A, B, R, U),
  range_substraction(A, B, P, V))))).
