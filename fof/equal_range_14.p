...
fof(equal_range_14, conjecture, ![S, T, P, Q]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(tuple2(a, b)), P) = P)
  => ((sort(set(tuple2(a, b)), Q) = Q) => ((mem(set(tuple2(b, a)), inverse(b,
  a, P), infix_plmngt(a, b, T, S)) & (mem(set(tuple2(b, a)), inverse(b, a,
  Q), infix_plmngt(a, b, T, S)) & infix_eqeq(tuple2(a, b),
  range_restriction(b, a, Q, ran(b, a, P)), range_restriction(b, a, P, ran(b,
  a, Q))))) => infix_eqeq(b, ran(b, a, inter(tuple2(a, b), P, Q)), inter(b,
  ran(b, a, P), ran(b, a, Q))))))))).
