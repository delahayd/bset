...
tff(equal_image_1, conjecture, ![A : $tType, B : $tType, C : $tType]: ![S:
  set(A), U:set(A), T:set(B), V:set(C), P:set(tuple2(A, B)), Q:
  set(tuple2(B, C))]: ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & (mem(set(tuple2(B, C)), Q, relation(B, C, T, V)) & subset(A, U, S)))
  => infix_eqeq(C, image(A, C, semicolon(A, B, C, P, Q), U), image(B, C, Q,
  image(A, B, P, U))))).
