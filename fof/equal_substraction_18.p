...
fof(equal_substraction_18, conjecture, ![S, U, T, R]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => ((sort(set(tuple2(a,
  b)), R) = R) => ((subset(a, U, S) & (mem(set(tuple2(a, b)), R, relation(b,
  a, S, T)) & subset(a, dom(b, a, R), U))) => infix_eqeq(tuple2(a, b),
  domain_substraction(b, a, U, R), empty(tuple2(a, b))))))))).
