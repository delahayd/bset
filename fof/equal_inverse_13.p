...
fof(equal_inverse_13, conjecture, ![S, T, X, Y]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(a, X) = X) => ((sort(b, Y) = Y)
  => ((mem(a, X, S) & mem(b, Y, T)) => infix_eqeq(tuple2(b, a), inverse(b, a,
  singleton(tuple2(a, b), tuple21(a, b, X, Y))), singleton(tuple2(b, a),
  tuple21(b, a, Y, X))))))))).
