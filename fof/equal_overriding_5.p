...
fof(equal_overriding_5, conjecture, ![S, T, V, X, Y]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(b), V) = V) => ((sort(a, X) = X)
  => ((sort(b, Y) = Y) => ((mem(a, X, S) & (subset(b, V, T) & mem(b, Y, T)))
  => infix_eqeq(tuple2(a, b), infix_lspl(b, a, times(b, a, singleton(a, X),
  V), singleton(tuple2(a, b), tuple21(a, b, X, Y))), singleton(tuple2(a, b),
  tuple21(a, b, X, Y)))))))))).
