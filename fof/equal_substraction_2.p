...
fof(equal_substraction_2, conjecture, ![S, U, T, R]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => ((sort(set(tuple2(a,
  b)), R) = R) => ((subset(a, U, S) & mem(set(tuple2(b, a)), inverse(b, a,
  R), infix_plmngt(a, b, T, S))) => infix_eqeq(tuple2(a, b),
  domain_substraction(b, a, U, R), range_substraction(b, a, R, image(b, a, R,
  U))))))))).
