...
tff(equal_inverse_9, conjecture, ![A : $tType, B : $tType, C : $tType,
  D : $tType]: ![S:set(A), T:set(B), U:set(C), V:set(D), P:set(tuple2(A, B)),
  Q:set(tuple2(C, D))]: ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & mem(set(tuple2(C, D)), Q, relation(C, D, U, V)))
  => infix_eqeq(tuple2(tuple2(B, D), tuple2(A, C)), inverse(tuple2(A, C),
  tuple2(B, D), parallel_product(A, B, C, D, P, Q)), parallel_product(B, A,
  D, C, inverse(A, B, P), inverse(C, D, Q))))).
