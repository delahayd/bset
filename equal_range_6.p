...
tff(equal_range_6, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), T:set(B), R:set(tuple2(A, B))]: ((subset(A, U, S)
  & mem(set(tuple2(A, B)), R, relation(A, B, S, T))) => infix_eqeq(B, ran(A,
  B, domain_restriction(A, B, U, R)), image(A, B, R, U)))).
