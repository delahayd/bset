...
fof(inclusion_substraction_1, conjecture, ![S, U, T, R]: ((sort(set(a),
  S) = S) => ((sort(set(a), U) = U) => ((sort(set(b), T) = T)
  => ((sort(set(tuple2(a, b)), R) = R) => ((subset(a, U, S)
  & mem(set(tuple2(a, b)), R, relation(b, a, S, T))) => subset(tuple2(a, b),
  domain_substraction(b, a, U, R), R))))))).
