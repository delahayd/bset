...
tff(monot_proj_1, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), T:set(B), V:set(B)]: ((subset(A, U, S) & subset(B, V, T))
  => subset(tuple2(tuple2(A, B), A), prj1(A, B, tuple21(set(B), set(A), U,
  V)), prj1(A, B, tuple21(set(B), set(A), S, T))))).
