...
fof(monot_semicolon, conjecture, ![S, T, U, H, P, K, Q]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(tuple2(a, b)), H) = H) => ((sort(set(tuple2(a, b)), P) = P)
  => ((sort(set(tuple2(b, c)), K) = K) => ((sort(set(tuple2(b, c)), Q) = Q)
  => ((mem(set(tuple2(a, b)), H, relation(b, a, S, T)) & mem(set(tuple2(b,
  c)), K, relation(c, b, T, U))) => ((subset(tuple2(a, b), P, H)
  & subset(tuple2(b, c), Q, K)) => subset(tuple2(a, c), semicolon(c, b, a, P,
  Q), semicolon(c, b, a, H, K)))))))))))).
