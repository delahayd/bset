...
tff(monot_ran, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:set(B),
  P:set(tuple2(A, B)), R:set(tuple2(A, B))]: (mem(set(tuple2(A, B)), P,
  relation(A, B, S, T)) => (subset(tuple2(A, B), R, P) => subset(B, ran(A,
  B, R), ran(A, B, P))))).
