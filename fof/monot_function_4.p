...
fof(monot_function_4, conjecture, ![S, T, V]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(b), V) = V) => (subset(b, V, T)
  => subset(set(tuple2(a, b)), infix_gtmngt(b, a, S, V), infix_gtmngt(b, a,
  S, T))))))).
