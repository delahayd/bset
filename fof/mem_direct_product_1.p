...
fof(mem_direct_product_1, conjecture, ![S, T, U, P, Q]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(tuple2(a, b)), P) = P) => ((sort(set(tuple2(a, c)), Q) = Q)
  => ((mem(set(tuple2(a, b)), P, relation(b, a, S, T)) & mem(set(tuple2(a,
  c)), Q, relation(c, a, S, U))) => mem(set(tuple2(a, tuple2(b, c))),
  direct_product(c, b, a, P, Q), relation(tuple2(b, c), a, S, times(c, b, T,
  U)))))))))).
