...
tff(equal_substraction_20, conjecture, ![A : $tType, B : $tType]: ![S:
  set(A), U:set(A), V:set(A), T:set(B), R:set(tuple2(A, B))]: ((subset(A, U,
  S) & (subset(A, V, S) & mem(set(tuple2(A, B)), R, relation(A, B, S, T))))
  => infix_eqeq(tuple2(A, B), domain_substraction(A, B, union(A, U, V), R),
  inter(tuple2(A, B), domain_substraction(A, B, U, R), domain_substraction(A,
  B, V, R))))).
