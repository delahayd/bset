...
fof(equal_inverse_6, conjecture, ![S, U, T, R]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => ((sort(set(tuple2(a,
  b)), R) = R) => ((subset(a, U, S) & mem(set(tuple2(a, b)), R, relation(b,
  a, S, T))) => infix_eqeq(tuple2(b, a), inverse(b, a, domain_substraction(b,
  a, U, R)), range_substraction(a, b, inverse(b, a, R), U)))))))).
