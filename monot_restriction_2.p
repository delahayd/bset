...
tff(monot_restriction_2, conjecture, ![A : $tType, B : $tType]: ![S:set(A),
  T:set(B), U:set(B), V:set(B), P:set(tuple2(A, B)), R:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T)) & subset(B, V, T))
  => ((subset(tuple2(A, B), R, P) & subset(B, U, V))
  => subset(tuple2(A, B), range_restriction(A, B, R, U), range_restriction(A,
  B, P, V))))).
