...
fof(equal_substraction_10, conjecture, ![S, U, T, V, P, Q]: ((sort(set(a),
  S) = S) => ((sort(set(a), U) = U) => ((sort(set(b), T) = T)
  => ((sort(set(c), V) = V) => ((sort(set(tuple2(a, b)), P) = P)
  => ((sort(set(tuple2(a, c)), Q) = Q) => ((subset(a, U, S)
  & (mem(set(tuple2(a, b)), P, relation(b, a, S, T)) & mem(set(tuple2(a, c)),
  Q, relation(c, a, S, V)))) => infix_eqeq(tuple2(a, tuple2(b, c)),
  direct_product(c, b, a, domain_substraction(b, a, U, P),
  domain_substraction(c, a, U, Q)), domain_substraction(tuple2(b, c), a, U,
  direct_product(c, b, a, P, Q))))))))))).
