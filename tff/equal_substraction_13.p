...
tff(equal_substraction_13, conjecture, ![A : $tType, B : $tType]: ![S:
  set(A), U:set(A), T:set(B), P:set(tuple2(A, B)), Q:set(tuple2(A, B))]:
  ((subset(A, U, S) & (mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & mem(set(tuple2(A, B)), Q, relation(A, B, S, T))))
  => infix_eqeq(tuple2(A, B), domain_substraction(A, B, U,
  union(tuple2(A, B), P, Q)), union(tuple2(A, B), domain_substraction(A,
  B, U, P), domain_substraction(A, B, U, Q))))).
