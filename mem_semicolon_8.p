...
tff(mem_semicolon_8, conjecture, ![A : $tType, B : $tType, C : $tType]: ![S:
  set(A), T:set(B), U:set(C), F:set(tuple2(A, B)), G:set(tuple2(B, C))]:
  ((mem(set(tuple2(A, B)), F, infix_gtmngtgt(A, B, S, T))
  & mem(set(tuple2(B, C)), G, infix_gtmngtgt(B, C, T, U)))
  => mem(set(tuple2(A, C)), semicolon(A, B, C, F, G), infix_gtmngtgt(A, C, S,
  U)))).
