...
fof(equal_substraction_4, conjecture, ![S, U, V]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(a), V) = V) => ((subset(a, U, S)
  & subset(a, V, S)) => infix_eqeq(tuple2(a, a), domain_substraction(a, a, U,
  id(a, V)), id(a, diff(a, V, U)))))))).
