...
fof(equal_inverse_7, conjecture, ![S, T, V, R]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(b), V) = V) => ((sort(set(tuple2(a,
  b)), R) = R) => ((mem(set(tuple2(a, b)), R, relation(b, a, S, T))
  & subset(b, V, T)) => infix_eqeq(tuple2(b, a), inverse(b, a,
  range_substraction(b, a, R, V)), domain_substraction(a, b, V, inverse(b, a,
  R))))))))).
