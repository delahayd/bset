...
tff(equal_restriction_9, conjecture, ![A : $tType, B : $tType]: ![S:set(A),
  U:set(A), T:set(B), P:set(tuple2(A, B)), Q:set(tuple2(A, B))]:
  ((subset(A, U, S) & (mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & mem(set(tuple2(A, B)), Q, relation(A, B, S, T))))
  => infix_eqeq(tuple2(A, B), domain_restriction(A, B, U, infix_lspl(A, B, P,
  Q)), infix_lspl(A, B, domain_restriction(A, B, U, P), domain_restriction(A,
  B, U, Q))))).
