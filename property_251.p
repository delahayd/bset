...
tff(property_251, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), F:set(tuple2(A, B))]: (mem(set(tuple2(A, B)), F, infix_mnmngt(A,
  B, S, T)) <=> ![X:A]: (mem(A, X, S)
  => mem(set(tuple2(A, B)), domain_restriction(A, B, singleton(A, X), F),
  infix_mnmngt(A, B, singleton(A, X), T))))).
