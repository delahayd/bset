...
fof(mem_overriding_7, conjecture, ![S, T, F, G]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(tuple2(a, b)), F) = F)
  => ((sort(set(tuple2(a, b)), G) = G) => ((mem(set(tuple2(a, b)), F,
  infix_plmngt(b, a, S, T)) & mem(set(tuple2(a, b)), G, infix_mnmngtgt(b, a,
  S, T))) => mem(set(tuple2(a, b)), infix_lspl(b, a, F, G), infix_mnmngtgt(b,
  a, S, T)))))))).
