...
fof(equal_semicolon_6, conjecture, ![S, T, V, U, R, P]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(b), V) = V)
  => ((sort(set(c), U) = U) => ((sort(set(tuple2(a, b)), R) = R)
  => ((sort(set(tuple2(b, c)), P) = P) => ((mem(set(tuple2(a, b)), R,
  relation(b, a, S, T)) & (subset(b, V, T) & mem(set(tuple2(b, c)), P,
  relation(c, b, T, U)))) => infix_eqeq(tuple2(a, c), semicolon(c, b, a, R,
  domain_restriction(c, b, V, P)), semicolon(c, b, a, range_restriction(b, a,
  R, V), P)))))))))).
