...
tff(monot_semicolon, conjecture, ![A : $tType, B : $tType, C : $tType]: ![S:
  set(A), T:set(B), U:set(C), H:set(tuple2(A, B)), P:set(tuple2(A, B)), K:
  set(tuple2(B, C)), Q:set(tuple2(B, C))]: ((mem(set(tuple2(A, B)), H,
  relation(A, B, S, T)) & mem(set(tuple2(B, C)), K, relation(B, C, T, U)))
  => ((subset(tuple2(A, B), P, H) & subset(tuple2(B, C), Q, K))
  => subset(tuple2(A, C), semicolon(A, B, C, P, Q), semicolon(A, B, C, H,
  K))))).
