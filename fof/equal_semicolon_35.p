...
fof(equal_semicolon_35, conjecture, ![S, T, U, V, W, P, Q, H, K]:
  ((sort(set(a), S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(d), V) = V) => ((sort(set(e), W) = W) => ((sort(set(tuple2(a,
  b)), P) = P) => ((sort(set(tuple2(a, c)), Q) = Q) => ((sort(set(tuple2(b,
  d)), H) = H) => ((sort(set(tuple2(c, e)), K) = K) => ((mem(set(tuple2(a,
  b)), P, relation(b, a, S, T)) & (mem(set(tuple2(a, c)), Q, relation(c, a,
  S, U)) & (mem(set(tuple2(b, d)), H, relation(d, b, T, V))
  & mem(set(tuple2(c, e)), K, relation(e, c, U, W)))))
  => infix_eqeq(tuple2(a, tuple2(d, e)), semicolon(tuple2(d, e), tuple2(b,
  c), a, direct_product(c, b, a, P, Q), parallel_product(e, c, d, b, H, K)),
  direct_product(e, d, a, semicolon(d, b, a, P, H), semicolon(e, c, a, Q,
  K)))))))))))))).
