...
fof(mem_union_2, conjecture, ![S, T, F, G]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(tuple2(a, b)), F) = F)
  => ((sort(set(tuple2(a, b)), G) = G) => ((mem(set(tuple2(a, b)), F,
  infix_gtplgt(b, a, S, T)) & (mem(set(tuple2(a, b)), G, infix_gtplgt(b, a,
  S, T)) & (infix_eqeq(tuple2(a, b), domain_restriction(b, a, dom(b, a, G),
  F), domain_restriction(b, a, dom(b, a, F), G)) & infix_eqeq(tuple2(a, b),
  range_restriction(b, a, F, ran(b, a, G)), range_restriction(b, a, G, ran(b,
  a, F)))))) => mem(set(tuple2(a, b)), union(tuple2(a, b), F, G),
  infix_gtplgt(b, a, S, T)))))))).
