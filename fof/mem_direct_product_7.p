...
fof(mem_direct_product_7, conjecture, ![S, T, U, F, G]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(tuple2(a, b)), F) = F) => ((sort(set(tuple2(a, c)), G) = G)
  => ((mem(set(tuple2(a, b)), F, infix_gtmngtgt(b, a, S, T))
  & mem(set(tuple2(a, c)), G, infix_gtmngtgt(c, a, S, U)))
  => mem(set(tuple2(a, tuple2(b, c))), direct_product(c, b, a, F, G),
  infix_gtmngtgt(tuple2(b, c), a, S, times(c, b, T, U)))))))))).
