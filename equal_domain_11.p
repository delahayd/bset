...
tff(equal_domain_11, conjecture, ![A : $tType, B : $tType, C : $tType]: ![S:
  set(A), T:set(B), U:set(C), P:set(tuple2(A, B)), Q:set(tuple2(A, C))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & mem(set(tuple2(A, C)), Q, relation(A, C, S, U))) => infix_eqeq(A, dom(A,
  tuple2(B, C), direct_product(A, B, C, P, Q)), inter(A, dom(A, B, P), dom(A,
  C, Q))))).
