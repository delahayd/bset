...
fof(monot_function_5, conjecture, ![S, U, T]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => (subset(a, U, S)
  => subset(set(tuple2(a, b)), infix_plmngtgt(b, a, U, T), infix_plmngtgt(b,
  a, S, T))))))).
