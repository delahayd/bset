...
fof(equal_domain_4, conjecture, ![S, T, U, P, Q]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(c), U) = U) => ((sort(set(tuple2(a,
  b)), P) = P) => ((sort(set(tuple2(b, c)), Q) = Q) => ((mem(set(tuple2(a,
  b)), P, relation(b, a, S, T)) & (mem(set(tuple2(b, c)), Q, relation(c, b,
  T, U)) & subset(b, ran(b, a, P), dom(c, b, Q)))) => infix_eqeq(a, dom(c, a,
  semicolon(c, b, a, P, Q)), dom(b, a, P))))))))).
