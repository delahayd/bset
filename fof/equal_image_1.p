...
fof(equal_image_1, conjecture, ![S, U, T, V, P, Q]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => ((sort(set(c),
  V) = V) => ((sort(set(tuple2(a, b)), P) = P) => ((sort(set(tuple2(b, c)),
  Q) = Q) => ((mem(set(tuple2(a, b)), P, relation(b, a, S, T))
  & (mem(set(tuple2(b, c)), Q, relation(c, b, T, V)) & subset(a, U, S)))
  => infix_eqeq(c, image(c, a, semicolon(c, b, a, P, Q), U), image(c, b, Q,
  image(b, a, P, U))))))))))).
