...
fof(equal_substraction_24, conjecture, ![S, T, V, F]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(b), V) = V) => ((sort(set(tuple2(a,
  b)), F) = F) => ((mem(set(tuple2(a, b)), F, infix_plmngt(b, a, S, T))
  & subset(b, V, T)) => infix_eqeq(tuple2(a, b), domain_substraction(b, a,
  image(a, b, inverse(b, a, F), V), F), range_substraction(b, a, F,
  V)))))))).
