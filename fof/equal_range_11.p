...
fof(equal_range_11, conjecture, ![S, T, U, P, Q]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(c), U) = U) => ((sort(set(tuple2(a,
  b)), P) = P) => ((sort(set(tuple2(a, c)), Q) = Q) => ((mem(set(tuple2(a,
  b)), P, relation(b, a, S, T)) & mem(set(tuple2(a, c)), Q, relation(c, a, S,
  U))) => infix_eqeq(tuple2(b, c), ran(tuple2(b, c), a, direct_product(c, b,
  a, P, Q)), semicolon(c, a, b, inverse(b, a, P), Q))))))))).
