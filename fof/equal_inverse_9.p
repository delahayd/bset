...
fof(equal_inverse_9, conjecture, ![S, T, U, V, P, Q]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(c), U) = U) => ((sort(set(d),
  V) = V) => ((sort(set(tuple2(a, b)), P) = P) => ((sort(set(tuple2(c, d)),
  Q) = Q) => ((mem(set(tuple2(a, b)), P, relation(b, a, S, T))
  & mem(set(tuple2(c, d)), Q, relation(d, c, U, V)))
  => infix_eqeq(tuple2(tuple2(b, d), tuple2(a, c)), inverse(tuple2(b, d),
  tuple2(a, c), parallel_product(d, c, b, a, P, Q)), parallel_product(c, d,
  a, b, inverse(b, a, P), inverse(d, c, Q))))))))))).
