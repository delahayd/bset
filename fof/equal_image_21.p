...
fof(equal_image_21, conjecture, ![S, U, V, T, R]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(a), V) = V) => ((sort(set(b),
  T) = T) => ((sort(set(tuple2(a, b)), R) = R) => ((mem(set(tuple2(b, a)),
  inverse(b, a, R), infix_plmngt(a, b, T, S)) & (subset(a, U, S) & subset(a,
  V, S))) => infix_eqeq(b, image(b, a, R, inter(a, U, V)), inter(b, image(b,
  a, R, U), image(b, a, R, V)))))))))).
