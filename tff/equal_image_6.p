...
tff(equal_image_6, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), V:set(A), T:set(B), R:set(tuple2(A, B))]: ((subset(A, U, S)
  & (mem(set(tuple2(A, B)), R, relation(A, B, S, T)) & subset(A, V, S)))
  => infix_eqeq(B, image(A, B, domain_substraction(A, B, U, R), V), image(A,
  B, R, diff(A, U, V))))).
