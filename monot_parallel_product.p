...
tff(monot_parallel_product, conjecture, ![A : $tType, B : $tType, C : $tType,
  D : $tType]: ![S:set(A), T:set(B), U:set(C), V:set(D), H:set(tuple2(A, B)),
  P:set(tuple2(A, B)), K:set(tuple2(C, D)), Q:set(tuple2(C, D))]:
  ((mem(set(tuple2(A, B)), H, relation(A, B, S, T))
  & mem(set(tuple2(C, D)), K, relation(C, D, U, V)))
  => ((subset(tuple2(A, B), P, H) & subset(tuple2(C, D), Q, K))
  => subset(tuple2(tuple2(A, C), tuple2(B, D)), parallel_product(A, B, C,
  D, P, Q), parallel_product(A, B, C, D, H, K))))).
