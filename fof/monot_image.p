...
fof(monot_image, conjecture, ![S, U, V, T, P, R]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(a), V) = V) => ((sort(set(b),
  T) = T) => ((sort(set(tuple2(a, b)), P) = P) => ((sort(set(tuple2(a, b)),
  R) = R) => ((mem(set(tuple2(a, b)), P, relation(b, a, S, T)) & subset(a, V,
  S)) => ((subset(tuple2(a, b), R, P) & subset(a, U, V)) => subset(b,
  image(b, a, R, U), image(b, a, P, V))))))))))).
