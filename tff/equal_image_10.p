...
tff(equal_image_10, conjecture, ![A : $tType, B : $tType, C : $tType,
  D : $tType]: ![S:set(A), U:set(A), T:set(B), W:set(C), V:set(C), Z:
  set(D), P:set(tuple2(A, B)), Q:set(tuple2(C, D))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & (mem(set(tuple2(C, D)), Q, relation(C, D, W, Z)) & (subset(A, U, S)
  & subset(C, V, W)))) => infix_eqeq(tuple2(B, D), image(tuple2(A, C),
  tuple2(B, D), parallel_product(A, B, C, D, P, Q), times(A, C, U, V)),
  times(B, D, image(A, B, P, U), image(C, D, Q, V))))).
