...
fof(equal_semicolon_23, conjecture, ![S, T, V, U, P, R]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(b), V) = V)
  => ((sort(set(c), U) = U) => ((sort(set(tuple2(a, b)), P) = P)
  => ((sort(set(tuple2(b, c)), R) = R) => ((mem(set(tuple2(a, b)), P,
  relation(b, a, S, T)) & (subset(b, V, T) & mem(set(tuple2(b, c)), R,
  relation(c, b, T, U)))) => infix_eqeq(tuple2(a, c), semicolon(c, b, a,
  range_substraction(b, a, P, V), R), semicolon(c, b, a, P,
  domain_substraction(c, b, V, R))))))))))).
