...
tff(equal_image_8, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), T:set(B), P:set(tuple2(A, B)), Q:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & (mem(set(tuple2(A, B)), Q, relation(A, B, S, T)) & subset(A, U, S)))
  => infix_eqeq(B, image(A, B, infix_lspl(A, B, P, Q), U), union(B, image(A,
  B, domain_substraction(A, B, dom(A, B, Q), P), U), image(A, B, Q, U))))).
