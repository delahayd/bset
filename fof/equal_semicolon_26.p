...
fof(equal_semicolon_26, conjecture, ![S, T, U, P, Q, R]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(tuple2(a, b)), P) = P) => ((sort(set(tuple2(a, b)), Q) = Q)
  => ((sort(set(tuple2(b, c)), R) = R) => ((mem(set(tuple2(a, b)), P,
  relation(b, a, S, T)) & (mem(set(tuple2(a, b)), Q, relation(b, a, S, T))
  & mem(set(tuple2(c, b)), inverse(c, b, R), infix_plmngt(b, c, U, T))))
  => infix_eqeq(tuple2(a, c), semicolon(c, b, a, inter(tuple2(a, b), P, Q),
  R), inter(tuple2(a, c), semicolon(c, b, a, P, R), semicolon(c, b, a, Q,
  R))))))))))).
