...
fof(equal_semicolon_3, conjecture, ![S, T, U, V, P, Q, R]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(d), V) = V) => ((sort(set(tuple2(b, c)), P) = P)
  => ((sort(set(tuple2(c, d)), Q) = Q) => ((sort(set(tuple2(a, b)), R) = R)
  => ((mem(set(tuple2(a, b)), R, relation(b, a, S, T)) & (mem(set(tuple2(b,
  c)), P, relation(c, b, T, U)) & mem(set(tuple2(c, d)), Q, relation(d, c, U,
  V)))) => infix_eqeq(tuple2(a, d), semicolon(d, b, a, R, semicolon(d, c, b,
  P, Q)), semicolon(d, c, a, semicolon(c, b, a, R, P), Q))))))))))).
