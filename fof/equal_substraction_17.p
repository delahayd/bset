...
fof(equal_substraction_17, conjecture, ![S, U, T, X, Y]: ((sort(set(a),
  S) = S) => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => ((sort(a,
  X) = X) => ((sort(b, Y) = Y) => ((subset(a, U, S) & (mem(a, X, S) & (mem(b,
  Y, T) & mem(a, X, U)))) => infix_eqeq(tuple2(a, b), domain_substraction(b,
  a, U, singleton(tuple2(a, b), tuple21(a, b, X, Y))), empty(tuple2(a,
  b)))))))))).
