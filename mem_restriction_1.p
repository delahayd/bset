...
tff(mem_restriction_1, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), U:set(A), R:set(tuple2(A, B))]: ((subset(A, U, S)
  & mem(set(tuple2(A, B)), R, relation(A, B, S, T)))
  => mem(set(tuple2(A, B)), domain_restriction(A, B, U, R), relation(A, B, U,
  T)))).
