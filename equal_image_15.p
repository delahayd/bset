...
tff(equal_image_15, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), T:set(B), X:A, Y:B]: ((mem(A, X, S) & (mem(B, Y, T) & (subset(A, U,
  S) & mem(A, X, U)))) => infix_eqeq(B, image(A,
  B, singleton(tuple2(A, B), tuple21(B, A, X, Y)), U), singleton(B, Y)))).
