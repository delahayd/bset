...
tff(mem_union_2, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), F:set(tuple2(A, B)), G:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), F, infix_gtplgt(A, B, S, T))
  & (mem(set(tuple2(A, B)), G, infix_gtplgt(A, B, S, T))
  & (infix_eqeq(tuple2(A, B), domain_restriction(A, B, dom(A, B, G), F),
  domain_restriction(A, B, dom(A, B, F), G))
  & infix_eqeq(tuple2(A, B), range_restriction(A, B, F, ran(A, B, G)),
  range_restriction(A, B, G, ran(A, B, F))))))
  => mem(set(tuple2(A, B)), union(tuple2(A, B), F, G), infix_gtplgt(A, B, S,
  T)))).
