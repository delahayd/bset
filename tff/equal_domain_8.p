...
tff(equal_domain_8, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), T:set(B), R:set(tuple2(A, B))]: ((subset(A, U, S)
  & mem(set(tuple2(A, B)), R, relation(A, B, S, T))) => infix_eqeq(A, dom(A,
  B, domain_substraction(A, B, U, R)), diff(A, dom(A, B, R), U)))).
