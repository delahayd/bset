...
fof(monot_restriction_2, conjecture, ![S, T, U, V, P, R]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(b), U) = U)
  => ((sort(set(b), V) = V) => ((sort(set(tuple2(a, b)), P) = P)
  => ((sort(set(tuple2(a, b)), R) = R) => ((mem(set(tuple2(a, b)), P,
  relation(b, a, S, T)) & subset(b, V, T)) => ((subset(tuple2(a, b), R, P)
  & subset(b, U, V)) => subset(tuple2(a, b), range_restriction(b, a, R, U),
  range_restriction(b, a, P, V))))))))))).
