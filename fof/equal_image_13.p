...
fof(equal_image_13, conjecture, ![S, U, T, P, Q]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => ((sort(set(tuple2(a,
  b)), P) = P) => ((sort(set(tuple2(a, b)), Q) = Q) => ((mem(set(tuple2(b,
  a)), inverse(b, a, P), infix_plmngt(a, b, T, S)) & (mem(set(tuple2(b, a)),
  inverse(b, a, Q), infix_plmngt(a, b, T, S)) & (subset(a, U, S)
  & infix_eqeq(tuple2(a, b), range_restriction(b, a, P, ran(b, a, Q)),
  range_restriction(b, a, Q, ran(b, a, P)))))) => infix_eqeq(b, image(b, a,
  inter(tuple2(a, b), P, Q), U), inter(b, image(b, a, P, U), image(b, a, Q,
  U)))))))))).
