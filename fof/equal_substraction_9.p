...
fof(equal_substraction_9, conjecture, ![S, U, T, P, Q]: ((sort(set(a),
  S) = S) => ((sort(set(a), U) = U) => ((sort(set(b), T) = T)
  => ((sort(set(tuple2(a, b)), P) = P) => ((sort(set(tuple2(a, b)), Q) = Q)
  => ((subset(a, U, S) & (mem(set(tuple2(a, b)), P, relation(b, a, S, T))
  & mem(set(tuple2(a, b)), Q, relation(b, a, S, T)))) => infix_eqeq(tuple2(a,
  b), domain_substraction(b, a, U, infix_lspl(b, a, P, Q)), infix_lspl(b, a,
  domain_substraction(b, a, U, P), domain_substraction(b, a, U, Q)))))))))).
