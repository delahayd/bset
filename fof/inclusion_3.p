...
fof(inclusion_3, conjecture, ![S, A, B, C]: ((sort(set(a), S) = S)
  => ((sort(set(a), A) = A) => ((sort(set(a), B) = B) => ((sort(set(a),
  C) = C) => ((subset(a, A, S) & (subset(a, B, S) & subset(a, C, S)))
  => (subset(a, inter(a, A, B), C) <=> subset(a, A, union(a, diff(a, S, B),
  C))))))))).
