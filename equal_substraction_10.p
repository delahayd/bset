...
tff(equal_substraction_10, conjecture, ![A : $tType, B : $tType, C : $tType]:
  ![S:set(A), U:set(A), T:set(B), V:set(C), P:set(tuple2(A, B)), Q:
  set(tuple2(A, C))]: ((subset(A, U, S) & (mem(set(tuple2(A, B)), P,
  relation(A, B, S, T)) & mem(set(tuple2(A, C)), Q, relation(A, C, S, V))))
  => infix_eqeq(tuple2(A, tuple2(B, C)), direct_product(A, B,
  C, domain_substraction(A, B, U, P), domain_substraction(A, C, U, Q)),
  domain_substraction(A, tuple2(B, C), U, direct_product(A, B, C, P, Q))))).
