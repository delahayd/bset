...
fof(mem_direct_product_5, conjecture, ![S, T, U, F, G]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(tuple2(a, b)), F) = F) => ((sort(set(tuple2(a, c)), G) = G)
  => ((mem(set(tuple2(a, b)), F, infix_gtmngt(b, a, S, T))
  & mem(set(tuple2(a, c)), G, infix_gtmngt(c, a, S, U))) => mem(set(tuple2(a,
  tuple2(b, c))), direct_product(c, b, a, F, G), infix_gtmngt(tuple2(b, c),
  a, S, times(c, b, T, U)))))))))).
