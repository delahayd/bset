...
tff(equal_semicolon_28, conjecture, ![A : $tType, B : $tType, C : $tType]:
  ![S:set(A), T:set(B), U:set(C), X:A, Y:B, R:set(tuple2(B, C))]: ((mem(A, X,
  S) & (mem(B, Y, T) & mem(set(tuple2(B, C)), R, relation(B, C, T, U))))
  => infix_eqeq(tuple2(A, C), semicolon(A, B,
  C, singleton(tuple2(A, B), tuple21(B, A, X, Y)), R), times(A,
  C, singleton(A, X), image(B, C, R, singleton(B, Y)))))).
