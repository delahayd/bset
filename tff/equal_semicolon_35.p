...
tff(equal_semicolon_35, conjecture, ![A : $tType, B : $tType, C : $tType,
  D : $tType, E : $tType]: ![S:set(A), T:set(B), U:set(C), V:set(D), W:
  set(E), P:set(tuple2(A, B)), Q:set(tuple2(A, C)), H:set(tuple2(B, D)), K:
  set(tuple2(C, E))]: ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & (mem(set(tuple2(A, C)), Q, relation(A, C, S, U))
  & (mem(set(tuple2(B, D)), H, relation(B, D, T, V))
  & mem(set(tuple2(C, E)), K, relation(C, E, U, W)))))
  => infix_eqeq(tuple2(A, tuple2(D, E)), semicolon(A, tuple2(B, C),
  tuple2(D, E), direct_product(A, B, C, P, Q), parallel_product(B, D, C,
  E, H, K)), direct_product(A, D, E, semicolon(A, B, D, P, H), semicolon(A,
  C, E, Q, K))))).
