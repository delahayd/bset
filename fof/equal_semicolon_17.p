...
fof(equal_semicolon_17, conjecture, ![S, T, U, V, F, P, Q]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(d), V) = V) => ((sort(set(tuple2(a, b)), F) = F)
  => ((sort(set(tuple2(b, c)), P) = P) => ((sort(set(tuple2(b, d)), Q) = Q)
  => ((mem(set(tuple2(a, b)), F, infix_plmngt(b, a, S, T))
  & (mem(set(tuple2(b, c)), P, relation(c, b, T, U)) & mem(set(tuple2(b, d)),
  Q, relation(d, b, T, V)))) => infix_eqeq(tuple2(a, tuple2(c, d)),
  semicolon(tuple2(c, d), b, a, F, direct_product(d, c, b, P, Q)),
  direct_product(d, c, a, semicolon(c, b, a, F, P), semicolon(d, b, a, F,
  Q)))))))))))).
