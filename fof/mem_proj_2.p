...
fof(mem_proj_2, conjecture, ![S, T]: ((sort(set(a), S) = S) => ((sort(set(b),
  T) = T) => mem(set(tuple2(tuple2(a, b), b)), prj2(b, a, tuple21(set(a),
  set(b), S, T)), infix_mnmngt(b, tuple2(a, b), times(b, a, S, T), T))))).
