...
tff(equal_domain_7, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), V:set(B), R:set(tuple2(A, B))]: ((mem(set(tuple2(A, B)), R,
  relation(A, B, S, T)) & subset(B, V, T)) => infix_eqeq(A, dom(A,
  B, range_restriction(A, B, R, V)), image(B, A, inverse(A, B, R), V)))).
