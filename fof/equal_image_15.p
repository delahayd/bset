...
fof(equal_image_15, conjecture, ![S, U, T, X, Y]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => ((sort(a, X) = X)
  => ((sort(b, Y) = Y) => ((mem(a, X, S) & (mem(b, Y, T) & (subset(a, U, S)
  & mem(a, X, U)))) => infix_eqeq(b, image(b, a, singleton(tuple2(a, b),
  tuple21(a, b, X, Y)), U), singleton(b, Y))))))))).
