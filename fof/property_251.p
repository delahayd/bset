...
fof(property_251, conjecture, ![S, T, F]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(tuple2(a, b)), F) = F)
  => ((mem(set(tuple2(a, b)), F, infix_mnmngt(b, a, S, T)) | ![X]: (mem(a, X,
  S) => mem(set(tuple2(a, b)), domain_restriction(b, a, singleton(a, X), F),
  infix_mnmngt(b, a, singleton(a, X), T)))) => (mem(set(tuple2(a, b)), F,
  infix_mnmngt(b, a, S, T)) & ![X]: ((sort(a, X) = X) => (mem(a, X, S)
  => mem(set(tuple2(a, b)), domain_restriction(b, a, singleton(a, X), F),
  infix_mnmngt(b, a, singleton(a, X), T)))))))))).
