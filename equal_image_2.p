...
tff(equal_image_2, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), T:set(B), R:set(tuple2(A, B))]: ((mem(set(tuple2(B, A)), inverse(A,
  B, R), infix_plmngt(B, A, T, S)) & subset(A, U, dom(A, B, R)))
  => infix_eqeq(A, image(A, A, semicolon(A, B, A, R, inverse(A, B, R)), U),
  U))).
