...
tff(equal_semicolon_9, conjecture, ![A : $tType, B : $tType, C : $tType]:
  ![S:set(A), T:set(B), U:set(C), W:set(C), R:set(tuple2(A, B)), P:
  set(tuple2(B, C))]: ((mem(set(tuple2(A, B)), R, relation(A, B, S, T))
  & (mem(set(tuple2(B, C)), P, relation(B, C, T, U)) & subset(C, W, U)))
  => infix_eqeq(tuple2(A, C), semicolon(A, B, C, R, range_substraction(B,
  C, P, W)), range_substraction(A, C, semicolon(A, B, C, R, P), W)))).
