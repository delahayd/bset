...
fof(monot_proj_1, conjecture, ![S, U, T, V]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => ((sort(set(b),
  V) = V) => ((subset(a, U, S) & subset(b, V, T)) => subset(tuple2(tuple2(a,
  b), a), prj1(b, a, tuple21(set(a), set(b), U, V)), prj1(b, a,
  tuple21(set(a), set(b), S, T))))))))).
