...
fof(monot_overriding, conjecture, ![S, T, H, Q, P]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(tuple2(a, b)), H) = H)
  => ((sort(set(tuple2(a, b)), Q) = Q) => ((sort(set(tuple2(a, b)), P) = P)
  => ((mem(set(tuple2(a, b)), H, relation(b, a, S, T)) & mem(set(tuple2(a,
  b)), Q, relation(b, a, S, T))) => (subset(tuple2(a, b), P, H)
  => subset(tuple2(a, b), infix_lspl(b, a, P, Q), infix_lspl(b, a, H,
  Q)))))))))).
