...
fof(equal_semicolon_16, conjecture, ![S, T, U, V, R]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(b), U) = U) => ((sort(set(c),
  V) = V) => ((sort(set(tuple2(a, b)), R) = R) => ((mem(set(tuple2(a, b)), R,
  relation(b, a, S, T)) & subset(b, U, T)) => infix_eqeq(tuple2(a, c),
  semicolon(c, b, a, R, times(c, b, U, V)), times(c, a, image(a, b,
  inverse(b, a, R), U), V))))))))).
