...
fof(equal_semicolon_11, conjecture, ![S, T, U, R, P, Q]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(tuple2(a, b)), R) = R) => ((sort(set(tuple2(b, c)), P) = P)
  => ((sort(set(tuple2(b, c)), Q) = Q) => ((mem(set(tuple2(a, b)), R,
  relation(b, a, S, T)) & (mem(set(tuple2(b, c)), P, relation(c, b, T, U))
  & mem(set(tuple2(b, c)), Q, relation(c, b, T, U)))) => infix_eqeq(tuple2(a,
  c), semicolon(c, b, a, R, union(tuple2(b, c), P, Q)), union(tuple2(a, c),
  semicolon(c, b, a, R, P), semicolon(c, b, a, R, Q))))))))))).
