...
fof(union_diff_distributivity, conjecture, ![A, B, C]: ((sort(set(a), A) = A)
  => ((sort(set(a), B) = B) => ((sort(set(a), C) = C) => infix_eqeq(a,
  diff(a, union(a, A, B), C), union(a, diff(a, A, C), diff(a, B, C))))))).
