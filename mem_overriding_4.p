...
tff(mem_overriding_4, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), F:set(tuple2(A, B)), G:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), F, infix_plmngt(A, B, S, T))
  & mem(set(tuple2(A, B)), G, infix_plmngtgt(A, B, S, T)))
  => mem(set(tuple2(A, B)), infix_lspl(A, B, F, G), infix_plmngtgt(A, B, S,
  T)))).
