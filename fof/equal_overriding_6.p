...
fof(equal_overriding_6, conjecture, ![S, U, T, X, Y]: ((sort(set(a), S) = S)
  => ((sort(set(a), U) = U) => ((sort(set(b), T) = T) => ((sort(a, X) = X)
  => ((sort(b, Y) = Y) => ((subset(a, U, S) & (mem(b, Y, T) & mem(a, X, S)))
  => infix_eqeq(tuple2(a, b), infix_lspl(b, a, times(b, a, U, singleton(b,
  Y)), singleton(tuple2(a, b), tuple21(a, b, X, Y))), times(b, a, union(a, U,
  singleton(a, X)), singleton(b, Y)))))))))).
