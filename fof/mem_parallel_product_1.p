...
fof(mem_parallel_product_1, conjecture, ![S, T, U, V, P, Q]: ((sort(set(a),
  S) = S) => ((sort(set(b), T) = T) => ((sort(set(c), U) = U)
  => ((sort(set(d), V) = V) => ((sort(set(tuple2(a, b)), P) = P)
  => ((sort(set(tuple2(c, d)), Q) = Q) => ((mem(set(tuple2(a, b)), P,
  relation(b, a, S, T)) & mem(set(tuple2(c, d)), Q, relation(d, c, U, V)))
  => mem(set(tuple2(tuple2(a, c), tuple2(b, d))), parallel_product(d, c, b,
  a, P, Q), relation(tuple2(b, d), tuple2(a, c), times(c, a, S, U), times(d,
  b, T, V))))))))))).
