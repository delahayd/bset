...
tff(equal_semicolon_26, conjecture, ![A : $tType, B : $tType, C : $tType]:
  ![S:set(A), T:set(B), U:set(C), P:set(tuple2(A, B)), Q:set(tuple2(A, B)),
  R:set(tuple2(B, C))]: ((mem(set(tuple2(A, B)), P, relation(A, B, S, T))
  & (mem(set(tuple2(A, B)), Q, relation(A, B, S, T))
  & mem(set(tuple2(C, B)), inverse(B, C, R), infix_plmngt(C, B, U, T))))
  => infix_eqeq(tuple2(A, C), semicolon(A, B, C, inter(tuple2(A, B), P, Q),
  R), inter(tuple2(A, C), semicolon(A, B, C, P, R), semicolon(A, B, C, Q,
  R))))).
