...
tff(equal_image_20, conjecture, ![A : $tType, B : $tType]: ![S:set(A), U:
  set(A), V:set(A), T:set(B), R:set(tuple2(A, B))]:
  ((mem(set(tuple2(A, B)), R, relation(A, B, S, T)) & (subset(A, U, S)
  & subset(A, V, S))) => infix_eqeq(B, image(A, B, R, union(A, U, V)),
  union(B, image(A, B, R, U), image(A, B, R, V))))).
