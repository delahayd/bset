...
fof(monot_function_2, conjecture, ![S, T, U, V]: ((sort(set(a), S) = S)
  => ((sort(set(b), T) = T) => ((sort(set(a), U) = U) => ((sort(set(b),
  V) = V) => ((subset(a, U, S) & subset(b, V, T)) => subset(set(tuple2(a,
  b)), infix_gtplgt(b, a, U, V), infix_gtplgt(b, a, S, T)))))))).
