...
tff(equal_inverse_13, conjecture, ![A : $tType, B : $tType]: ![S:set(A), T:
  set(B), X:A, Y:B]: ((mem(A, X, S) & mem(B, Y, T))
  => infix_eqeq(tuple2(B, A), inverse(A,
  B, singleton(tuple2(A, B), tuple21(B, A, X, Y))),
  singleton(tuple2(B, A), tuple21(A, B, Y, X))))).
